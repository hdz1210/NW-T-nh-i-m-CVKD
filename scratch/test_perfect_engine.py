...
th_map = {} # code.lower() -> list of rules
general_rules = [] # code is 'Tất cả'
...
for r in range(4, ws_th.max_row + 1):
    ...
    c_lower = cur_code.lower()
    if c_lower in ('', '*', 'tất cả', 'tat ca', 'all'):
        general_rules.append(rule)
    else:
        if c_lower not in th_map: th_map[c_lower] = []
        th_map[c_lower].append(rule)

# Add Month 9 Rules to general_rules
m9_general = [
    # Masterise
...
general_rules.extend(m9_general)
...
def evaluate_row_full(row):
    ...
    if loai_quy == 'Quỹ Chéo':
        ...
        if du_an_key in f2_map and month_key in f2_map[du_an_key]:
            ...
            # Check general Quỹ Chéo rules in general_rules matching month_key
            matched_qcheo = []
        ...
        # 2. QUY NW
        du_an_key = du_an.lower()
        candidates = th_map.get(du_an_key, [])
        if not candidates:
            for code, lst in th_map.items():
                if du_an_key in code or code in du_an_key:
                    candidates = lst
                    break
        ...
        if candidates:
            ...
            if matched_row:
                if month_key in matched_row['monthScores']:
                    ...
                    # Fallback only for older months to nearest month <= month_key
                    found_score = None
                    for m in th_months:
                        if m['key'] <= month_key and m['key'] in matched_row['monthScores']:
                            found_score = matched_row['monthScores'][m['key']]
                            break
                    if found_score is None:
                        for m in th_months:
                            if m['key'] in matched_row['monthScores']:
                                found_score = matched_row['monthScores'][m['key']]
                                break
                    base_score = found_score if found_score is not None else 2
                else:
                    # In month >= 2026-09, if this specific project has no score for month_key:
                    # Check if general rules for month 9 match!
                    matched_row = None # fall through to general rules!

        if matched_row is None:
            # Check general_rules
            gen_candidates = []
            for r in general_rules:
                if r.get('fund') == 'Quỹ Chéo': continue
                ...
                gen_candidates.append((spec, r, r['monthScores'][month_key]))

            if gen_candidates:
                gen_candidates.sort(key=lambda x: x[0], reverse=True)
                best_rule = gen_candidates[0][1]
                base_score = gen_candidates[0][2]
...
for r_idx, row in enumerate(ws_data.iter_rows(min_row=2, max_row=ws_data.max_row, values_only=True)):
    total += 1
    sheet_score = row[23]
    calc_score = evaluate_row_full(row)
