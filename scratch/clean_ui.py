...
def main():
    ...
    # 1. Simplify onEditScoreMonthChange and onEditScoreMonthChangeF2
    old_edit_fns = """    function onEditScoreMonthChange(rowIdx) {
    ...
    new_edit_fns = """    function onEditScoreMonthChange(rowIdx) {
      const select = document.getElementById('editScoreMonth');
      const scoreInput = document.getElementById('editScoreVal');
      const badge = document.getElementById('editScoreMonthBadge');
      const row = configData.thRows.find(r => r.rowIdx === rowIdx);
    ...
    if old_edit_fns not in content:
        print("Error: old_edit_fns not found!")
        return False
    content = content.replace(old_edit_fns, new_edit_fns)

    # 2. Clean openEditRowModal TH block
    old_th_modal = """        title.innerText = `Sửa Dự Án: ${row.code} (${row.name})`;
    ...
    new_th_modal = """        title.innerText = `Sửa Dự Án: ${row.code} (${row.name})`;
        const submitBtnTH = document.getElementById('btnEditRowSubmit');
        if (submitBtnTH) submitBtnTH.innerText = 'Lưu Cập Nhật';
        const selMonthOnToolbar = document.getElementById('monthFilter') ? document.getElementById('monthFilter').value : 'ALL';
        const latestM = (configData.thMonths && configData.thMonths.length > 0) ? configData.thMonths[0] : null;
    ...
    if old_th_modal not in content:
        print("Error: old_th_modal not found!")
        return False
    content = content.replace(old_th_modal, new_th_modal)

    # 3. Clean openEditRowModal TH HTML (remove checkbox and alert div)
    old_th_html = """          <!-- Cập nhật điểm cho tháng cụ thể hoặc tất cả -->
    ...
    new_th_html = """          <!-- Cập nhật điểm cho tháng cụ thể hoặc tất cả -->
          <div style="background: var(--bg-muted); border: 1px solid var(--border-subtle); border-radius: var(--radius-md); padding: 12px; display: flex; flex-direction: column; gap: 8px;">
            <div style="display: flex; justify-content: space-between; align-items: center;">
              <span style="font-size: var(--text-xs); font-weight: 800; color: var(--text-secondary); text-transform: uppercase;">
                CẬP NHẬT ĐIỂM CHO THÁNG
              </span>
              <span id="editScoreMonthBadge" class="badge" style="background:#fff7ed; color:#9a3412; border:1px solid #fdba74; font-weight:700;">
                ⚡ CHỈ THÁNG ${latestM ? latestM.display : ''}
              </span>
            </div>
            <div class="form-row-2">
              <div class="form-group" style="margin-bottom:0;">
    ...
    if old_th_html not in content:
        print("Error: old_th_html not found!")
        return False
    content = content.replace(old_th_html, new_th_html)

    # 4. Clean openEditRowModal F2 block
    old_f2_modal = """        const selMonthOnToolbar = document.getElementById('monthFilter') ? document.getElementById('monthFilter').value : 'ALL';
    ...
    new_f2_modal = """        const selMonthOnToolbar = document.getElementById('monthFilter') ? document.getElementById('monthFilter').value : 'ALL';
        const latestF2M = (configData.f2Months && configData.f2Months.length > 0) ? configData.f2Months[0] : null;
        const targetMonthDateStrF2 = (selMonthOnToolbar !== 'ALL') ? selMonthOnToolbar : (latestF2M ? latestF2M.dateStr : 'ALL');
    ...
    if old_f2_modal not in content:
        print("Error: old_f2_modal not found!")
        return False
    content = content.replace(old_f2_modal, new_f2_modal)

    # 5. Clean openEditRowModal F2 HTML (remove checkbox and alert div)
    old_f2_html = """          <!-- Cấu hình điểm & Tháng áp dụng -->
          <div style="background: var(--bg-muted); border: 1px solid var(--border-subtle); border-radius: var(--radius-md); padding: 12px; display: flex; flex-direction: column; gap: 8px;">
            <div style="display: flex; justify-content: space-between; align-items: center;">
              <span style="font-size: var(--text-xs); font-weight: 800; color: var(--text-secondary); text-transform: uppercase;">
                CẬP NHẬT ĐIỂM CHO THÁNG
    ...
    new_f2_html = """          <!-- Cấu hình điểm & Tháng áp dụng -->
          <div style="background: var(--bg-muted); border: 1px solid var(--border-subtle); border-radius: var(--radius-md); padding: 12px; display: flex; flex-direction: column; gap: 8px;">
            <div style="display: flex; justify-content: space-between; align-items: center;">
              <span style="font-size: var(--text-xs); font-weight: 800; color: var(--text-secondary); text-transform: uppercase;">
                CẬP NHẬT ĐIỂM CHO THÁNG
              </span>
              <span id="editF2ScoreMonthBadge" class="badge" style="background:#fff7ed; color:#9a3412; border:1px solid #fdba74; font-weight:700;">
                ⚡ CHỈ THÁNG ${latestF2M ? latestF2M.display : ''}
              </span>
            </div>
            <div class="form-row-2">
              <div class="form-group" style="margin-bottom:0;">
    ...
    if old_f2_html not in content:
        print("Error: old_f2_html not found!")
        return False
    content = content.replace(old_f2_html, new_f2_html)

    # 6. Automatic month isolation in submitEditRow for TH and F2
    ...
    new_submit_th = """        // Check if score changed in modal
        ...
        if (row && newScoreVal !== '') {
          ...
          if (editScoreMonth === 'ALL') {
            ...
            // Tự động: Chọn tháng cụ thể thì CHỈ ghi điểm vào tháng đó, toàn bộ các tháng khác tự động xóa trống để bảo toàn lịch sử!
            configData.thMonths.forEach(m => {
              if (m.dateStr === editScoreMonth) {
                row.scores[m.dateStr] = parsedScore;
                modifiedCellsTH.set(`${rowIdx}_${m.colIdx}`, { rowIdx, colIdx: m.colIdx, score: parsedScore });
    ...
    print("Success: Fully simplified! No checkboxes, no alert banners. Completely automatic month isolation!")
    return True
